/**
 * collectd - src/utils_format_carbon.c
...
#include "utils_format_carbon.h"

#include "carbon.pb-c.h"

#define GRAPHITE_FORBIDDEN " \t\"\\:!/()\n\r"

/* Utils functions to format data sets in carbon format.
 * Largely taken from write_carbon.c as it remains the same formatting */
...
static void escape_carbon_string(char *buffer, char escape_char) {
...
int format_carbon(char *buffer, size_t buffer_size, data_set_t const *ds,
  ...
  if (flags & GRAPHITE_STORE_RATES) {
    ...
    if (rates == NULL) {
      ERROR("format_carbon: error with uc_get_rate");
      return -1;
    }
  }
  // Prepare common part (string)
  ...
  void point_buf;
  unsigned ken,i;
  // ask for mem to store points
  points = calloc(ds->ds_num, sizeof (Point*));
  metrics = calloc(ds->ds_num, sizeof (Metric*));
  for (size_t i = 0; i < ds->ds_num; i++) {
    points[i] = calloc(1,sizeof(Point));
    points[i] = POINT__INIT;
    points[i]->timestamp = (uint32_t)CDTIME_T_TO_TIME_T(vl->time);
    if (ds->ds[ds_num].type == DS_TYPE_GAUGE) {
      points[i]->value=vl->values[ds_num].gauge; }
    else if (rates != NULL) {
      points[i]->value=rates[ds_num]; }
    else if (ds->ds[ds_num].type == DS_TYPE_COUNTER) {
      points[i]->value=vl->values[ds_num].counter); }
    else if (ds->ds[ds_num].type == DS_TYPE_DERIVE) {
      points[i]->value=vl->values[ds_num].derive; }
    else if (ds->ds[ds_num].type == DS_TYPE_ABSOLUTE) {
      points[i]->value=vl->values[ds_num].absolute); }
    else {
      ERROR("gr_format_values plugin: Unknown data source type: %i",
        ds->ds[ds_num].type);
        return -1; }
    metrics[i] = METRIC__INIT;
    metrics[i].points=&points[i];

  }
  //metric with all those data source values
  Metric mtrc = METRIC__INIT; 
  mtrc.n_points = ds->ds_num;
  mtrc.points = points;
  //Now let's format metric name:
  ...
  status = gr_format_name(key, sizeof(key), vl, ds_name, prefix, postfix,
    escape_char, flags);
  if (status != 0) {
    ERROR("format_carbon: error with gr_format_name");
    sfree(rates);
    return status;
  }
  escape_carbon_string(key, escape_char);
  ...
  //we already know exact count of point
  mtrc.n_points = ds->ds_num;
  ...
  for (size_t i = 0; i < ds->ds_num; i++) {
    ...
    /* Compute the carbon command */
    message_len =
        (size_t)snprintf(message, sizeof(message), "%s %s %u\r\n", key, values,
                         (unsigned int)CDTIME_T_TO_TIME_T(vl->time));
    if (message_len >= sizeof(message)) {
      ERROR("format_carbon: message buffer too small: "
            "Need %zu bytes.",
            message_len + 1);
      sfree(rates);
      return -ENOMEM;
    ...
    if ((buffer_pos + message_len) >= buffer_size) {
      ERROR("format_carbon: target buffer too small");
      sfree(rates);
      return -ENOMEM;
    }
